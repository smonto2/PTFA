...
class ProbabilisticPLS:
    ...
    def fitted(self, X, Y, standardize = False):
        ...
        # Dispatch to relevant lower-level function
        if no_missing:
            return self.fitted_ppls(X, Y, standardize)
        else:
            return self.fitted_ppls_missing(X, X_missing_index, Y, Y_missing_index, standardize)
    ...
    def predict(self, X, standardize = False):
        ...
        # Dispatch to relevant lower-level function
        if no_missing:
            return self.predict_ppls(X, standardize)
        else:
            return self.predict_ppls_missing(X, X_missing_index, standardize)
    ...
    def fitted_ppls(self, X, Y, standardize):
        # Center and scale predictors and targets separately before stacking
        # q = self.Q_.shape[0]
        if standardize:
            X = (X - X.mean(axis = 0)) / X.std(axis = 0)
    ...
    def predict_ppls(self, X, standardize):
        # Center and scale predictors and targets separately before stacking
        # q = self.Q_.shape[0]
        if standardize:
            X = (X - X.mean(axis = 0)) / X.std(axis = 0)
    ...
    def fitted_ppls_missing(self, X, X_missing_index, Y, Y_missing_index, standardize):
        # Center and scale predictors and targets separately if necessary
        p = self.P_.shape[0]
        if standardize:
            X = (X - X.mean(axis = 0, where = np.logical_not(X_missing_index))) / X.std(axis = 0, where = np.logical_not(X_missing_index))
    ...
    def predict_ppls_missing(self, X, X_missing_index, standardize):
        # Center and scale predictors and targets separately before stacking
        p = self.P_.shape[0]
        if standardize:
            X = (X - X.mean(axis = 0, where = np.logical_not(X_missing_index))) / X.std(axis = 0, where = np.logical_not(X_missing_index))
